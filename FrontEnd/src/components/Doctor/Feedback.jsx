...
function Feedback() {
  ...
  const [form, setForm] = useState({ displayName: '', role: 'Doctor', message: '' });
  const [loading, setLoading] = useState(false);
  const [submitted, setSubmitted] = useState(false);

  const roles = ['Doctor', 'User', 'Other'];
  ...
  if (submitted) {
    return (
      <div className="page" style={{ maxWidth: '600px', margin: '0 auto', textAlign: 'center', padding: '80px 20px' }}>
        ...
        <button className="btn btn-v btn-sm" onClick={() => { setSubmitted(false); setRating(0); setForm({ displayName: '', role: 'Doctor', message: '' }); }}>
          Submit Another
        </button>
      </div>
    );
    ...
    <div className="page" style={{ maxWidth: '680px', margin: '0 auto' }}>
      <div className="ph">
        <div className="ph-row">
          <div>
            ...
            <p>Your experience helps improve Smriti AI for clinical use.</p>
          </div>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{ fontSize: '13px', color: 'var(--b3)', margin: 0, lineHeight: '1.6' }}>
      ...
      <div className="card">
        <div style={{ marginBottom: '24px' }}>
          <label style={{ fontSize: '13px', fontWeight: '600', color: 'var(--b3)', display: 'block', marginBottom: '12px' }}>
            ...
            {[1,2,3,4,5].map(i => (
              <button
                ...
                style={{
                  ...
                  border: 'none',
                  cursor: 'pointer',
                  color: i <= (hovered || rating) ? '#f59e0b' : '#374151',
        ...
        <div className="g2" style={{ marginBottom: '20px' }}>
          <div className="field">
            <label className="flabel">Your Name <span style={{ color: 'var(--b4)', fontWeight: 400 }}>(optional)</span></label>
            <input
              className="finput"
              placeholder="e.g. Dr. Riya Sharma"
        ...
        <div className="field" style={{ marginBottom: '24px' }}>
          ...
          <textarea
            className="finput"
            rows="5"
            placeholder="Tell us how Smriti AI has helped you in your clinical workflow. What features do you love? What would you improve?"
            style={{ resize: 'vertical' }}
            value={form.message}
            onChange={e => setForm({ ...form, message: e.target.value })}
          />
          <div style={{ fontSize: '11px', color: 'var(--b4)', marginTop: '4px', textAlign: 'right' }}>
            {form.message.length} characters
          </div>
        </div>

        <div style={{ fontSize: '12px', color: 'var(--b4)', marginBottom: '20px', display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
          <span>🔒</span>
        ...
        <button className="btn btn-c" onClick={handleSubmit} disabled={loading} style={{ width: '100%', padding: '14px' }}>
