...
function scopeFile(filePath, scopeClass) {
    let content = fs.readFileSync(filePath, 'utf8');

    content = content.replace(/:root\s*\{/g, `.${scopeClass} {`);

    content = content.replace(/\[data-dark="true"\]\s*\{/g, `[data-dark="true"]:has(.${scopeClass}), [data-dark="true"] .${scopeClass} {`);

    content = content.replace(/\[data-dark="true"\]\s+\./g, `[data-dark="true"] .${scopeClass} .`);

    content = content.replace(/^body\s*\{/gm, `.${scopeClass} {`);
}
